...
beforeAll(async () => {
  ...
  provideFileDecoration = (
    vscode.window.registerFileDecorationProvider as jest.Mock
...
describe("provider", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (shouldExclude as jest.Mock).mockReturnValue(false);
  });
  ...
  it("should return badge and tooltip for a normal file", async () => {
    ...
    (formatBadgeFileSize as jest.Mock).mockReturnValue("1K");
  ...
  it("should return undefined for excluded files", async () => {
    ...
    (shouldExclude as jest.Mock).mockReturnValue(true);
  ...
  it("should return undefined when cancelled after getFileSize", async () => {
    ...
    (getFileSize as jest.Mock).mockImplementation(async () => {
      cancelled = true;
      return 1024;
    });
  ...
  it("should return undefined when cancelled after getLineCounts", async () => {
    ...
    (getLineCounts as jest.Mock).mockImplementation(async () => {
      cancelled = true;
  ...
  it("should return badge and tooltip when lineCounts is null", async () => {
    ...
    (getFileSize as jest.Mock).mockResolvedValue(2048);
    (formatBadgeFileSize as jest.Mock).mockReturnValue("2K");
    (formatFileSize as jest.Mock).mockReturnValue("2 KB");
